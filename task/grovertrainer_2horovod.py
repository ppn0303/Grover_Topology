...
class GROVERMotifTrainer:
    def __init__(self,
        ...
        if self.enable_multi_gpu:
            self.optimizer = mgw.DistributedOptimizer(self.optimizer,
                                                      named_parameters=self.model.named_parameters())
            self.motif_optimizer = mgw.DistributedOptimizer(self.motif_optimizer,
                                                      named_parameters=self.motif_model.named_parameters())
    ...
    def save(self, epoch, file_path, name=None) -> str:
        ...
        output_encoder_path = file_path + name + "_encoder.ep%d" % epoch
        state_encoder = {
            'args': self.args,
            'state_dict': self.model.state_dict(),
            'optimizer': self.optimizer.state_dict(),
        ...
        torch.save(state_encoder, output_encoder_path)
    ...
    def save_tmp(self, epoch, file_path, rank=0):
        ...
        :return:
        """
        store_temp_path = os.path.join(file_path, "tmp")
        if not os.path.exists(store_temp_path):
            os.makedirs(store_temp_path, exist_ok=True)
        store_path = os.path.join(store_temp_path, "model.%d" % rank)
        state = {
            'args': self.args,
            'state_dict': self.model.state_dict(),
            'motif_state_dict': self.motif_model.state_dict(),
        ...
        store_encoder_path = os.path.join(store_temp_path, "encodermodel.%d" % rank)
        state_encoder = {
            'args': self.args,
            'state_dict': self.model.state_dict(),
            'optimizer': self.optimizer.state_dict(),
        ...
        torch.save(state_encoder, store_encoder_path)


    # there is no motif state because this will use finetuning
    def save_best(self, epoch, file_path, rank=0):
        """
        Save the models for auto-restore during training.
        The model are stored in file_path/tmp folder and will replaced on each epoch.
        :param epoch: the epoch number.
        ...
        :return:
        """
        store_path = os.path.join(file_path, "best")
        if not os.path.exists(store_path):
            os.makedirs(store_path, exist_ok=True)
        save_path = os.path.join(store_path, "model.%d" % rank)
        state = {
            'args': self.args,
            'state_dict': self.model.state_dict(),
            'motif_state_dict': self.motif_model.state_dict(),
        ...
        store_encoder_path = os.path.join(store_path, "encodermodel.%d" % rank)
        state_encoder = {
            'args': self.args,
            'state_dict': self.model.state_dict(),
            'optimizer': self.optimizer.state_dict(),
