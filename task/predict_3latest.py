...
from grover.util.metrics import confusion_mat, confusion_mat_multi
...
def evaluate_predictions(preds: List[List[float]],
    ...
    """
    if arg.multi_class:
        results = roc_auc_score(targets, preds, multi_class='ovr')
        return [results]
...
def evaluate_predictions_cfm(preds: List[List[float]],
                         targets: List[List[float]],
                         num_tasks: int,
                         metric_func,
                         dataset_type: str,
                         args, logger = None) -> List[float]:
    """
    Evaluates predictions using a metric function and filtering out invalid targets.
    ...
    if args.multi_class and args.dataset_type=='classification':
        result_AUC.append(roc_auc_score(targets, preds, multi_class='ovr'))
        acc, rec, prec, f1s, tp, fp, tn, fn = confusion_mat_multi(targets, preds, args.multi_class_num)
        result_ACC.append(acc)
        result_REC.append(rec)
        result_PREC.append(prec)
        result_SPEC.append(0)
        ...
        if args.metric=='auc':
            ...
            assert('select auc or recall or f1')

        return results, result_AUC, result_ACC, result_REC, result_PREC, result_SPEC, result_F1, result_BA, result_TP, result_FP, result_TN, result_FN
    else:
        for i in range(num_tasks):
            # # Skip if all targets or preds are identical, otherwise we'll crash during classification
            if dataset_type == 'classification':
