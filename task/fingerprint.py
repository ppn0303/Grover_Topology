...
def do_generate_embvec(model: nn.Module,
                data: MoleculeDataset,
                args: Namespace,
                ) -> List[List[float]]:
    """
    ...
    for item in mol_loader:
        _, batch, features_batch, _, _ = item
        with torch.no_grad():
            atom_vec, bond_vec = model(batch, features_batch)
            atom_vectors.extend(atom_vec.data.cpu().numpy())
            bond_vectors.extend(bond_vec.data.cpu().numpy())
    return atom_vectors, bond_vectors


def generate_embvec(args: Namespace, logger: Logger = None) -> List[List[float]]:
    """
    Generate the embeddingvector

    :param logger:
    :param args: Arguments.
    :return: A list of lists of embedding vector.
    """
    ...
    atom_vec, bond_vec = do_generate_embvec(model=model,data=test_data,args=args)

    return atom_vec, bond_vec