...
def generate_and_save_moltrees(args: Namespace):
    ...
    #sometime BrokenpipeError awake, so need sequential
    if args.sequential:
        moltrees_map = map(make_moltree, mols)
    else:
        moltrees_map = Pool(10).imap(make_moltree, mols)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default=None,
                        help='Path to .p file where features will be saved as a compressed numpy archive')
    ...
    if args.save_path is None:
        args.save_path = args.data_path.split('csv')[0] + 'p'
    generate_and_save_moltrees(args)
